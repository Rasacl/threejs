...
gui.add(params, "uWaresFrequency").min(1).max(100).step(1).name("频率").onChange(value => {
    shaderMaterial.uniforms.uWaresFrequency.value = value;
})
gui.add(params, "uScale").min(0.1).max(1).step(0.01).name("缩放").onChange(value => {
    shaderMaterial.uniforms.uScale.value = value;
})
gui.add(params, "uNoiseFrequency").min(1).max(100).step(1).name("噪声频率").onChange(value => {

    shaderMaterial.uniforms.uNoiseFrequency.value = value;
})
gui.add(params, "uNoiseScale").min(0.1).max(5).step(0.01).name("噪声缩放").onChange(value => {
  shaderMaterial.uniforms.uNoiseScale.value = value;
})
gui.add(params, "uXzScale").min(0).max(5).step(0.1).name("xz缩放").onChange(value => {
  shaderMaterial.uniforms.uXzScale.value = value;
})
gui.addColor(params, "uHighColor").name("高色").onChange(value => {
  shaderMaterial.uniforms.uHighColor.value = new THREE.Color(value);
})
gui.addColor(params, "uLowColor").name("低色").onChange(value => {
  shaderMaterial.uniforms.uLowColor.value = new THREE.Color(value);
})
gui.add(params, "uXspeed").min(0).max(5).step(0.1).name("x速度").onChange(value => {
    shaderMaterial.uniforms.uXspeed.value = value;
})
gui.add(params, "uZspeed").min(0).max(5).step(0.1).name("z速度").onChange(value => {
    shaderMaterial.uniforms.uZspeed.value = value;
})
gui.add(params, "uNoiseSpeed").min(0).max(5).step(0.1).name("噪声速度").onChange(value => {
    shaderMaterial.uniforms.uNoiseSpeed.value = value;
})
gui.add(params, "uOpacity").min(0).max(1).step(0.01).name("透明度").onChange(value => {
...
//创建平面
const floor = new THREE.Mesh(new THREE.PlaneGeometry(1,1, 2048,2048), shaderMaterial);
